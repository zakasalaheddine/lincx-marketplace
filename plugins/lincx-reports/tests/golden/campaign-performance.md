...
3. `list_dimension_sets({ limit: 100, offset: 0, fields: ["dimensions"] })`.
4. (no tool) — dimension check: required `campaign`, `date`. Claude names the set it picked and that it carries both.
5. `report_query({ dimensionSetId, startDate: "2026-03-01", endDate: "2026-03-15", groupBy: ["date"], filter: { campaign: "Acme Spring" } })`.
